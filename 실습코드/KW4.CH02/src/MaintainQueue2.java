...
public class MaintainQueue2{
	private ArrayQueue<String> customers;
	public MaintainQueue2() {
		...
		while (choiceNum< choices.length-1) {
			...
			try {
				...
				case 4:
					...
					int countAhead= 0;
					String[] st=customers.toString().split(" ");
					for (int i=0; i<st.length; i++) { // iteration() method
						if (!st[i].equals(name)) {
							countAhead++;
						} else {
